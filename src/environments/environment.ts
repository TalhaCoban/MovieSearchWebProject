export const environment = {
    image_url: "https://image.tmdb.org/t/p/w500",
    trending_url: "https://api.themoviedb.org/3/trending/movie/day?language=en-US",
    upcoming_url: "https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=",
    popular_url: "https://api.themoviedb.org/3/movie/popular?language=en-US&page=",
    topRated_url: "https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=",
    nowPlaying_url: "https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=",
    api: "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIwMTM0NGUxZmFiNDM2OGY4MmI1MjVkMjNlNDgzYTUwZiIsIm5iZiI6MTcxOTY4NDI4Ni42MTg2MTQsInN1YiI6IjY1Y2Y5N2M5NjBjNzUxMDE3YjY5ZDM1YyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.760HsQJfRRmMOtx9-pASInGBI8JhrLtbpJutcvVi3BQ",

};
